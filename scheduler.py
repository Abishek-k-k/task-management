...
# ─────────────────────────────────────────────
# SAME credentials as app.py
# ─────────────────────────────────────────────
GMAIL_SENDER  = "a20114466@gmail.com"
GMAIL_APP_PWD = "jfir zpvm elbe wntp"

# ─────────────────────────────────────────────
# What time to send reminders every day
# ─────────────────────────────────────────────
SEND_TIME = "12:42"   # ← change to whatever time you want (24hr format)

# ─────────────────────────────────────────────
# Logging — shows activity in terminal
# ─────────────────────────────────────────────
logging.basicConfig(
    level=logging.INFO,
...
def get_all_users_with_due_tasks() -> list[dict]:
    """
    Query DB for every user who has at least one Pending task due today.
    Returns list of dicts: {user_id, username, email, tasks:[...]}
    """
...
def send_daily_reminders():
    """Called automatically every day at SEND_TIME."""
...
if __name__ == "__main__":
    ...
    # Optional: run once immediately on startup to test
    # send_daily_reminders()

    # Keep running forever
    while True:
        schedule.run_pending()
        time.sleep(30)   # check every 30 secon